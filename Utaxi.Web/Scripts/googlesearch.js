...
function ServiceBlockAreaList() {
    ...
    $.ajax({
        type: "POST",
        url: loadServiceBlockAreaListURL,
        data: {},
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            BlokedAreaList.push(data.BlockedAreaList);
        },
        failure: function () {
            console.log(result);
        },
...
function LoadGoogleMAP() {
    var bangaloreBounds = new google.maps.LatLngBounds(new google.maps.LatLng(12.32718, 77.193206), new google.maps.LatLng(14.139807, 78.711895));
    var InCityoptions = { bounds: bangaloreBounds, componentRestrictions: { country: "in" }, strictBounds: !0 };
    var OutStation = { bounds: bangaloreBounds, componentRestrictions: { country: "in" }, strictBounds: !0 };
    var markers = [];
    var map = new google.maps.Map(document.getElementById("divloadMap"), { mapTypeId: google.maps.MapTypeId.ROADMAP });
...
function CheckServiceAvailability(ServiceTypeID, PlaceName) {
    ...
    if (ServiceTypeID == "1") {
        if (BlokedAreaList.length > 0) {
            var Place = BlokedAreaList[0];
            for (var j = 0; j < Place.length; j++) {
                if (Place[j].AreaName == PlaceName) {
                    Status = !1;
                    j = Place.length;
                }
            }
